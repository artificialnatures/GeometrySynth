...
#define NODE_ADDRESS 2  // Change this so each module has unique addresss
#define PAYLOAD_SIZE 7 // Number of bytes  expected to be received by the master I2C node.  Change as needed
byte nodePayload[PAYLOAD_SIZE]; //data packet to be sent to master

int rx = 0; //serial rx
int tx = 1; //serial tx
byte bytearray[64]; //used to clear incoming data from serial port. currently not used - need it to use hardware serial comm?
boolean needsUpdate = false;
int bufferZone = 5; //bc inputs vary
//vars for 3 inputs
int currentVal;
int currentVal2;
int currentVal3;
...
void loop()
{
  ...
  if (readVal < currentVal - bufferZone || readVal > currentVal + bufferZone) {
    currentVal = readVal;
    needsUpdate = true;
  }
  if (readVal2 < currentVal2 - bufferZone || readVal2 > currentVal2 + bufferZone) {
    currentVal2 = readVal2;
    needsUpdate = true;
  }
  if (readVal3 < currentVal3 - bufferZone || readVal3 > currentVal3 + bufferZone) {
